...
==== ReminderSlot, ReminderItem に必要なコンストラクタを作成する

ReminderSlotクラスに引数なし・引数つきコンストラクタを追加する。
...
ReminderItemクラスに引数なし・引数つきコンストラクタを追加する。
...
[NOTE]
===============================
今回追加したコンストラクタは、Springフレームワークがデータベースの操作で利用する。
===============================
...
ReminderRepository.java に `findPreviousItem` メソッドを追加する。
...
public List<ReminderItem> findPreviousItem() {
  ...
  List<ReminderItem> list =
    jdbc.query(sql, new DataClassRowMapper<>(ReminderItem.class), now);   // <3>
  return list;
}
-----
////
...
-----
  public List<PushMessage> doPushReminderItems() {
    List<ReminderItem> previousItems =
        repository.findPreviousItem();　// <1>
    List<PushMessage> pushMessages = new ArrayList<>();
    // 本来であればUserIdごとにPushMessageをまとめるべきだが、
    // 授業レベルなので簡略化している
    for (ReminderItem item : previousItems) {  // <2>
  ...
  private PushMessage toPushMessage(ReminderItem item) { // <3>
