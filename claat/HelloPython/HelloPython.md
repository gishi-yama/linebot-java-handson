...
また、**連係できるのは自らのプログラムだけではない**。例えば、行政が公開しているコロナウィルスの公開などをWEB-APIで取得すれば、今日のコロナウィルス感染者数を答える機能を追加するなど、オープンデータを活用したシステムにも発展させることができる。(最近話題になった、筑波大学のシラバスシステムの代替版を学生が作った、というニュースもこのパターン)
